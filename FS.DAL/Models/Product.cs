using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using FS.Framework;
namespace FS.DAL.Models
{
  public class Product
  {

    [ColumnDescription("主键标识")]
    public int ID { get; set; }

    [ColumnDescription("标题")]
    public string Title { get; set; }

    [ColumnDescription("添加时间")]
    public DateTime AddDate { get; set; }

    [ColumnDescription("产品图")]
    public string Images { get; set; }

    [ColumnDescription("单位")]
    public int Unit { get; set; }

    [ColumnDescription("最小购买量")]
    public int MinNum { get; set; }

    [ColumnDescription("销售量")]
    public int SaleCount { get; set; }

    [ColumnDescription("浏览量")]
    public int Views { get; set; }

    [ColumnDescription("分类")]
    public string CategoryIDs { get; set; }

    [ColumnDescription("分类的父级关系")]
    public string ParentIDs { get; set; }

    [ColumnDescription("状态 1上架 2促销 3 清仓 4预售 0 下架 -4删除 -70 定时上架  -88待翻译 88已翻译")]
    public int State { get; set; }

    [ColumnDescription("仿货级别")]
    public int FakeLevel { get; set; }

    [ColumnDescription("产品长度")]
    public decimal ProductLength { get; set; }

    [ColumnDescription("产品宽度")]
    public decimal ProductWide { get; set; }

    [ColumnDescription("产品高")]
    public decimal ProductHeight { get; set; }

    [ColumnDescription("重量")]
    public decimal ProductWeight { get; set; }

    [ColumnDescription("备注")]
    public string Remark { get; set; }

    [ColumnDescription("原价")]
    public decimal Original { get; set; }

    [ColumnDescription("评论数")]
    public int Review { get; set; }

    [ColumnDescription("评论积分")]
    public int ReviewPoint { get; set; }

    [ColumnDescription("")]
    public int LotNum { get; set; }

    [ColumnDescription("售价")]
    public decimal Price { get; set; }

    [ColumnDescription("市场价")]
    public decimal MarketPrice { get; set; }

    [ColumnDescription("是否免运费 0非免 1免运费")]
    public int Shipping { get; set; }

    [ColumnDescription("站点id")]
    public int SiteID { get; set; }

    [ColumnDescription("净重")]
    public decimal ProductNetWeight { get; set; }

    [ColumnDescription("折扣")]
    public decimal Discount { get; set; }

    [ColumnDescription("库存量")]
    public int Stock { get; set; }

    [ColumnDescription("排序")]
    public int Sort { get; set; }

    [ColumnDescription("材质")]
    public string Material { get; set; }

    [ColumnDescription("色号")]
    public string Color { get; set; }

    [ColumnDescription("促销结束时间")]
    public DateTime EndDate { get; set; }

    [ColumnDescription("属性值")]
    public string TagID { get; set; }

    [ColumnDescription("简要描述")]
    public string Summary { get; set; }

    [ColumnDescription("详细描述")]
    public string Description { get; set; }

    [ColumnDescription("备货期")]
    public string PrepareDate { get; set; }

    [ColumnDescription("")]
    public int PromotionType { get; set; }

    [ColumnDescription("是否是新品 0不是 1 是")]
    public int IsNew { get; set; }

    [ColumnDescription("下架时间")]
    public DateTime OutDate { get; set; }

    [ColumnDescription("标记为新品的时间")]
    public DateTime IsNewDate { get; set; }

    [ColumnDescription("影棚")]
    public int StudioID { get; set; }

    [ColumnDescription("url 用的title")]
    public string TitleUrl { get; set; }

    [ColumnDescription("产品线")]
    public int SOURCE { get; set; }

    [ColumnDescription("是否是热卖")]
    public int IsHot { get; set; }

    [ColumnDescription("是否是24小时快发产品")]
    public int Is24H { get; set; }

    [ColumnDescription("")]
    public int PmsCategoryID { get; set; }

    [ColumnDescription("")]
    public int SPUID { get; set; }

    [ColumnDescription("")]
    public string ImageDescription { get; set; }

    [ColumnDescription("")]
    public int MaxNum { get; set; }

    [ColumnDescription("")]
    public int IsEngraved { get; set; }

    [ColumnDescription("")]
    public int IsGraphCustomization { get; set; }
  }
}
