...
namespace FS.DB.SqlServer
{
    public class SqlDbHelper
    {
        ...
        /// 连接字符串  
        /// </summary>  
        public static readonly string connectionString = ConfigurationManager.AppSettings["ConnString"].ToString();
        ...
        public static DataTable ExecuteDataTable(string safeSql)
        {
            using (SqlConnection Connection = new SqlConnection(connectionString))
            {
                ...
                SqlCommand cmd = new SqlCommand(safeSql, Connection);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.SelectCommand.CommandTimeout = 10000000;
                try
                {
                    da.Fill(ds);
                }
